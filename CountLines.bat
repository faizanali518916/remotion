...
"$files = Get-ChildItem -Recurse -Include *.ts,*.tsx -File ^| Where-Object { $_.FullName -notmatch '\\.next\\' -and $_.FullName -notmatch '\\.vscode\\' -and $_.FullName -notmatch '\\node_modules\\' }; ^
$table = @(); ^
$totalLines = 0; ^
...
Write-Host ('{0,-100} {1,10}' -f 'File', 'Lines') -ForegroundColor Cyan; ^
Write-Host ('{0,-100} {1,10}' -f '----', '-----') -ForegroundColor Cyan; ^
foreach ($row in $top50) { ^
    if ($row.Lines -gt 500) { $color='Red' } ^
    elseif ($row.Lines -gt 300) { $color='Yellow' } ^
    else { $color='White' } ^
    Write-Host ('{0,-100} {1,10}' -f $row.File, $row.Lines) -ForegroundColor $color; ^
}; ^
Write-Host ''; ^
