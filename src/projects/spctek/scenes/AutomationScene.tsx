	...
	return (
		...
		>
			...
			<div style={{ position: 'absolute', top: 72, left: 130, right: 130, textAlign: 'center' }}>
				<AnimatedText delay={0} style={{ fontSize: 64, fontWeight: 850, letterSpacing: 0, lineHeight: 1.08 }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
					{automationUseCases.slice(0, 4).map((item, index) => {
						...
						return (
							...
							>
								<div style={{ fontSize: 24, color: '#e2e8f0', fontWeight: 800 }}>{item}</div>
							</GlassCard>
						);
					})}
				</div>
				<div style={{ position: 'relative', height: 420 }}>
					<svg width="900" height="420" viewBox="0 0 900 420" style={{ position: 'absolute', left: -270, top: 0 }}>
						<path
					...
					>
						<div
							style={{
								...
								height: 112,
								borderRadius: 999,
								background: 'radial-gradient(circle, rgba(34,211,238,0.9), rgba(34,211,238,0.08) 68%)',
				...
				<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
					{automationUseCases.slice(4).map((item, index) => {
						...
						return (
							...
							>
								<div style={{ fontSize: 24, color: '#dcfce7', fontWeight: 800 }}>{item}</div>
							</GlassCard>
						);
					})}
				</div>
