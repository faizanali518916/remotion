...
const callouts = ['Process gaps identified', 'Manual tasks prioritized', 'Automation opportunities mapped'];
		...
		>
			...
			>
				<div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 30 }}>
					<div>
						<div style={{ color: '#67e8f9', fontSize: 18, fontWeight: 800 }}>OPERATIONS AUDIT</div>
						<div style={{ marginTop: 6, fontSize: 34, fontWeight: 850 }}>Diagnostic dashboard</div>
					</div>
					<div style={{ color: '#22c55e', fontSize: 22, fontWeight: 800 }}>SYSTEM SIGNAL FOUND</div>
				</div>
				<div
					style={{
						position: 'absolute',
						...
						width: 96,
						height: '100%',
						background: 'linear-gradient(90deg, transparent, rgba(34,211,238,0.24), transparent)',
				...
				{auditRows.map((row, index) => {
					...
					return (
						...
						>
							<div style={{ fontSize: 24, color: '#e2e8f0', fontWeight: 700 }}>{row.label}</div>
							<div
								style={{
									fontSize: 20,
			...
			>
				{callouts.map((callout, index) => {
					...
					return (
						...
						>
							<div style={{ color: '#bbf7d0', fontSize: 21, fontWeight: 800 }}>{callout}</div>
						</GlassCard>
					);
				})}
			</div>
